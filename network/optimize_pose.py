...
def optimize_pose(data, hand_contact_target, obj_contact_target, n_iter=250, lr=0.01, w_cont_hand=2, w_cont_obj=1,
                  save_history=True, ncomps=45, w_cont_asym=2, w_opt_trans=0.3, w_opt_pose=1, w_opt_rot=1,
    ...
    mano_model = ManoLayer(mano_root='/scratch/minjay/coma_taxonomy_prediction/data/mano', use_pca=True, ncomps=ncomps, side='right', flat_hand_mean=True, center_idx=0).to(device)
    # ManoLayer(mano_root='mano/models', use_pca=True, ncomps=ncomps, side='right', flat_hand_mean=False).to(device)

    if data['obj_sampled_idx'].numel() > 1:
        # obj_normals -> generate_pointnet_features 에 존재함 .
    ...
    #data['mesh_aug'] = Meshes(verts=data['object_full_verts'], faces=data['object_full_face'])
    #is_thin = mesh_is_thin(data['mesh_aug'].num_verts_per_mesh())

    is_thin = torch.zeros(batch_size).to(device)
    ...
    for it in range(n_iter):
        ...
        #mano_pose_out[:, :ncomps+3] += data['aug_pose']
        mano_pose_out[:, 3:ncomps+3] += data['aug_pose'][:,0,:ncomps]
        mano_pose_out[:, :3] += data['aug_rot'][:,0,:]

        tform_out = util.translation_to_tform(opt_vector[:, ncomps+3:ncomps+6] * w_opt_trans)

        hand_verts, hand_joints = util.forward_mano(mano_model, mano_pose_out, data['hand_beta'][:,0,:], [data['hand_mTc_aug'], tform_out])   # 2.2ms
        ...
        contact_obj, contact_hand = calculate_contact.calculate_contact_capsule(hand_verts, hand_normals, obj_verts, obj_normals, caps_top=caps_top, caps_bot=caps_bot, caps_rad=caps_rad, caps_on_hand=caps_on_hand, contact_norm_method=contact_norm_method)
        ...
        if w_pen_cost > 0 and it >= pen_it:
            pen_cost = calculate_contact.calculate_penetration_cost(hand_verts, hand_normals, data['obj_verts'], obj_normals_sampled, is_thin, contact_norm_method)

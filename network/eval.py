...
EXP_NAME = 'GANHAND_AUG_TRANS_incep'
...
MODEL_LOAD_PATH = '/scratch/minjay/s2contact/GANHAND_AUG_TRANS_0.05_ROT_0.1_incep_onehot_econder/checkpoint_100.pt'

#'/scratch/minjay/s2contact/NIA_AUG_TRANS_incep_onehot/checkpoint_020.pt' #'/scratch/minjay/s2contact/NIA_AUG_TRANS_incep/checkpoint_020.pt'

#/scratch/minjay/s2contact/NIA_AUG_TRANS_incep_onehot/checkpoint_020.pt
...
class GANHAND(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if REVERSE_DATASET :
            ...
            if int(ho_info['taxonomy']) in COMMON_KEY :
        ...
        # Number of classes (adjust this based on your specific case)
            ...
            if sample['taxonomy'] == -1 :
                one_hot_encoding = F.one_hot(torch.tensor([0]), num_classes=num_classes)
            else :
                one_hot_encoding = F.one_hot(torch.tensor([sample['taxonomy']]), num_classes=num_classes)
...
class ContactPose(Dataset):
    def __init__(self,dirpath,setup):
        ...
        self.mano_layer = ManoLayer(side='right', mano_root='/scratch/minjay/coma_taxonomy_prediction/data/mano', use_pca=False, flat_hand_mean=True, center_idx=0, ncomps=45, root_rot_mode="axisang", joint_rot_mode="axisang").to(self.device)



        self.mano_faces = self.mano_layer.th_faces

        #mano_layer = ManoLayer(side='right', mano_root='/scratch/minjay/coma_taxonomy_prediction/data/mano', use_pca=False, flat_hand_mean=True, center_idx=0, ncomps=45, root_rot_mode="axisang", joint_rot_mode="axisang")
...
def run(model, train_loader, test_loader, epochs, optimizer, scheduler, writer, exp_name, device, args_o):
    ...
    for epoch in range(1, epochs + 1):
        ...
        test_hand_contact_loss, test_obj_contact_loss , f1_score_hand, f1_score_obj, precision_hand, recall_hand, precision_obj, recall_obj = test(model, test_loader, epoch, 0, 100, device)
        ...
        if epoch % 5 == 0 :
            writer.save_checkpoint(model, optimizer, scheduler, epoch, exp_name)
        # if epoch % 10 == 0 :
...
def test(model, test_loader, epoch, start_idx, end_idx, device) :
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(start_idx,end_idx)) :
            ...
            allowed_index = (sample['taxonomy'] != -1).nonzero(as_tuple=True)[0]
            
            TP_hand = np.sum(np.logical_and(contact_pred_mask_hand == True, contact_gt_mask_hand == True)[allowed_index])
            FP_hand = np.sum(np.logical_and(contact_pred_mask_hand == True, contact_gt_mask_hand == False)[allowed_index])
            FN_hand = np.sum(np.logical_and(contact_pred_mask_hand == False, contact_gt_mask_hand == True)[allowed_index])

            TP_obj = np.sum(np.logical_and(contact_pred_mask_obj == True, contact_gt_mask_obj == True)[allowed_index])
            FP_obj = np.sum(np.logical_and(contact_pred_mask_obj == True, contact_gt_mask_obj == False)[allowed_index])
            FN_obj = np.sum(np.logical_and(contact_pred_mask_obj == False, contact_gt_mask_obj == True)[allowed_index])
            #precision = (contact_pred_map[mask_TP_and_FP] == contact_gt_map[mask_TP_and_FP]).mean()
...
def run_contactopt(args):
    ...
    for idx, data in enumerate(tqdm(test_50_dataloader)):
        ...
        with torch.no_grad():
            ...
            #mesh_hand = trimesh.Trimesh(vertices=gt_ho.hand_vertss, faces=mano_faces)
            #mesh_hand.export('w_hand_mesh2.obj')
            #mesh_obj = trimesh.Trimesh(vertices=gt_ho.obj_verts, faces=gt_ho.obj_faces)
            #mesh_obj.export('w_obj_mesh2.obj')
    ...
    for idx, data in enumerate(tqdm(test_50_dataloader)):
        ...
        if contactopt_args.rand_re > 1:    # If we desire random restarts
            ...
            for ttttt, re_it in enumerate(range(contactopt_args.rand_re)):
        ...
        #mesh_hand = trimesh.Trimesh(vertices=out_ho.hand_verts, faces=mano_faces)
        #mesh_hand.export('w_hand_mesh2.obj')
        #mesh_obj = trimesh.Trimesh(vertices=out_ho.obj_verts, faces=out_ho.obj_faces)
        #mesh_obj.export('w_obj_mesh2.obj')

...
class HandObject:
    ...
    def load_from_batch(self, hand_beta, hand_pose, hand_mTc, hand_contact, obj_contact, obj_mesh, idx=0, obj_rot=None):
        ...
        self.hand_beta = hand_beta[idx, 0, :].detach().cpu().numpy()
        self.hand_pose = hand_pose[idx, 0, :].detach().cpu().numpy()
    ...
    def run_mano(self):
        ...
        if self.hand_pose.shape[0] == 48:   # Special case when we're loading GT honnotate
            mano_model  = ManoLayer(mano_root='/scratch/minjay/coma_taxonomy_prediction/data/mano', use_pca=True, ncomps=45, side='right', flat_hand_mean=True, center_idx=0)
